...
void ofApp::draw(){
    ...
    if( draw_flag ){
        ...
        for(int i=0; i<num_of_dot; i++){
            ...
            ofDrawCircle(dots[i].x, dots[i].y, dot_diameter/2);
        }
        // 2nd week portion.
        ofSetLineWidth(2);
    }
...
void ofApp::keyPressed(int key){
    ...
    if (key == 'q'){
        ...
        free(lines);
        cout << "Memory for line segment has been freed.\n";
        free(dots);
...
void ofApp::keyReleased(int key){
    ...
    if (key == OF_KEY_RIGHT){
        ...
        cout << "Selcted Dot Coordinate is (" << dots[selection_dot].x << ", " << dots[selection_dot].y << ")" << endl;
    }
    if (key == OF_KEY_LEFT){
        selection_dot--;
        if(selection_dot < 0)
    ...
    for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it) {
        ...
        if(words.size() == 1){
            if( input_type == 0){ // Input for the number of lines.
                ...
                if(lines == nullptr)
                    lines = (LineSegment*)malloc(sizeof(LineSegment) * num_of_line);
            ...
            if( input_type == 0){ // Input for actual information of lines
                ...
                // 좌표값이 스크린을 벗어났는지 판단
                if (!screenRect.inside(x1, y1) || !screenRect.inside(x2, y2)) {
                    cout << x1 << ", " << y1 << '\n';
                    cout << x2 << ", " << y2 << '\n';
                    cout << "Out of Screen\n";
                    return;
