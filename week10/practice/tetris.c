...
int recommend(RecNode *root){
	...
	int rot, x, y, child_idx = 0, recScore;
	int i, j, cnt = 0, delete=0;
	...
	for(rot = 0; rot<NUM_OF_ROTATE; rot++) {
		for(x = -2; x<WIDTH; x++) { //block이 배열의 idx=2번째 부터 있는 경우도 있으므로 -2부터 시작
			...
			if(!CheckToMove(root->recField, nextBlock[root->lv], rot, 0, x)) continue;
			y = 0;
			while(CheckToMove(root->recField, nextBlock[root->lv], rot, y+1, x))
				y++;
			recScore = 0;
			root->child[child_idx] = (RecNode*)malloc(sizeof(RecNode));
			root->child[child_idx]->lv = root->lv + 1;
			for(i = 0; i<HEIGHT; i++) 
				for(j = 0; j<WIDTH; j++) 
					root->child[child_idx]->recField[i][j] = root->recField[i][j];

			AddBlockToField(root->child[child_idx]->recField, nextBlock[root->lv], rot, y, x, &recScore);
			recScore += DeleteLine(root->child[child_idx]->recField);

			if(root->child[child_idx]->lv < BLOCK_NUM)
				recScore += recommend(root->child[child_idx]);
